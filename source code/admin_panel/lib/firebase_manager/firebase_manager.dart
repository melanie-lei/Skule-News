...
class FirebaseManager {
  ...
  Future<FirebaseResponse> insertUserAccount(String id, String? name, String email) async {
  ...
  Future<FirebaseResponse> addUsers(List<List<dynamic>> usersList) async {
    ...
    for (int i = 1; i < usersList.length; i++) {
      if (usersList[i].length < 3) {
        ...
        response = response ??
                  FirebaseResponse(false, '${LocalizationString.pleaseUseValidCsv} [row $i]');
      }
      ...
      if (name == '' || email == '' || password == '') {
        ...
        response = response ?? 
                  FirebaseResponse(false, '${LocalizationString.csvMissingCells} [row $i]');
      }
      ...
      if (password.length < 6) {
        ...
        print(password);
        response = response ??
                  FirebaseResponse(false, '${LocalizationString.passwordTooShort} [row $i]');
      }
